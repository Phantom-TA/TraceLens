...
const index_js_1 = require("./packages/bundle-analyzer/src/index.js");
...
async function main() {
    ...
    const result = (0, index_js_1.analyze)({
        webpackStats: syntheticWebpackStats,
    ...
    // ── Save to disk ──────────────────────────────────────────────────────────
    const outputDir = (0, path_1.join)(".", "reports", "bundle");
    (0, fs_1.mkdirSync)(outputDir, { recursive: true });
    const sessionId = result.analyzedAt.replace(/[:.TZ-]/g, "").slice(0, 15);
    const outputPath = (0, path_1.join)(outputDir, `${sessionId}-bundle-analysis.json`);
