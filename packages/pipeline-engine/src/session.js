 ...
 */
export function createPipelineContext(config) {
...
export function markStageStart(stage) {
    stage.status = "running";
    stage.startedAt = new Date().toISOString();
}
export function markStageDone(stage, _durationMs) {
    stage.status = "done";
...
export function markStageSkipped(stage, _reason) {
    stage.status = "skipped";
    stage.startedAt = new Date().toISOString();
    stage.completedAt = stage.startedAt;
    stage.durationMs = 0;
}
export function markStageFailed(stage, error, _fatal) {
    stage.status = "failed";
    stage.completedAt = new Date().toISOString();
