...
export async function runAggregationStage(
  ...
  try {
    const routes: RouteIntelligenceResult[] = [];
...
function buildRouteResult(bundle: RouteArtifactBundle): RouteIntelligenceResult {
...
function isSuccess(ctx: PipelineContext): boolean {
  const stages = ctx.stages;
  return (
    stages.playwright.status !== "failed" &&
