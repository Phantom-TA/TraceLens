...
export async function runLighthouseStage(ctx: PipelineContext): Promise<void> {
  ...
  try {
    ...
    for (const routeSummary of result.routes) {
      ...
      const firstRun = routeSummary.runs.find((r: import("../../../lighthouse-runner/src/types.js").LighthouseRouteResult) => r.success);
      if (!firstRun) {
