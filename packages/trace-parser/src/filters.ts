 ...
 */
export function discoverRendererThread(
  events: RawTraceEvent[]
): RendererThread | null {
  // Pass 1: Collect all CrRendererMain thread candidates
  const candidates = new Map<string, { pid: number; tid: number }>();
  ...
  let bestThread: { pid: number; tid: number; navStart: number } | null = null;
 ...
 */
function inferRendererThreadFromNavStart(
  events: RawTraceEvent[]
): RendererThread | null {
  ...
  const pidCounts = new Map<number, { count: number; tid: number }>();
  for (const ev of events) {
  ...
  let maxPid: { pid: number; tid: number } | null = null;
  let maxCount = 0;
...
export function filterEvents(
  ...
  for (const ev of events) {
    ...
    if (BLOCKED_NAMES.has(ev.name)) continue;
    ...
    if (!onMainThread && !isCrossProcess && !onRendererProcess) continue;
    ...
    if (!hasAllowedCat && !isCrossProcess) continue;
 ...
 */
export function getEventDurationUs(ev: RawTraceEvent): number {
  return ev.dur ?? 0;
}
 ...
 */
export function tsToMs(ts: number, navStartUs: number): number {
 ...
 */
export function durToMs(durUs: number): number {
 ...
 */
export function classifyEventCategory(ev: RawTraceEvent): string {
  const name = ev.name;
  const cat = ev.cat ?? "";
 ...
 */
export function extractScriptUrl(ev: RawTraceEvent): string | null {
  const data = ev.args?.["data"] ?? ev.args?.["beginData"] ?? {};
  const url: string = data["url"] ?? data["scriptName"] ?? data["fileName"] ?? "";
  if (!url || url.startsWith("v8-compile-cache") || url === "blob:") return null;

  // Shorten data URLs
  if (url.startsWith("data:")) return "inline-script";
