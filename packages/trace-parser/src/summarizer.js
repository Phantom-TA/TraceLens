...
export function buildAISignals(params) {
    const signals = [];
    const { vitals, mainThread, longTasks, lcpCandidate, renderBlockers, hydration, scripting, rendering, bundle, correlations, frameworkDetection, } = params;
    ...
    if (hydration.detected) {
        ...
        signals.push(`${fw} hydration detected${pct}${method}: ${hydration.durationMs}ms` +
