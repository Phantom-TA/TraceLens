...
export interface LHRAudit {
  ...
  displayValue?: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  details?: { items?: any[]; type?: string; [key: string]: any };
  title?: string;
}
...
export interface HAREntry {
  ...
  timings?: { wait?: number; receive?: number; send?: number };
  time?: number;
}

// ─── Intermediate Extracted Signals ───────────────────────────────────────────
...
export interface RendererThread {
  pid: number;
  tid: number;
  navigationStart: number; // µs — used as t=0 reference
}
...
export interface BundleSignals {
  ...
  topScripts: Array<{ url: string; evaluationMs: number }>;
}
