...
import {
  discoverRendererThread,
  filterEvents,
  tsToMs,
} from "./filters.js";

import {
  computeMainThreadSummary,
  extractLongTasks,
} from "./extractors/long-tasks.js";

import {
  extractLCPFromLHR,
  extractLCPFromTrace,
} from "./extractors/lcp.js";

import {
  extractRenderBlockersFromLHR,
  extractRenderBlockersFromTrace,
} from "./extractors/render-blocking.js";

import {
  detectHydration,
  detectHydrationFromLHR,
} from "./extractors/hydration.js";

import {
  computeBundleSignals,
  extractRenderingTimeline,
  extractScriptingBottlenecks,
  extractScriptingFromLHR,
} from "./extractors/scripting.js";
 ...
 * ```
 */
export function parse(input: ParseInput): ParsedTraceBottlenecks {
  ...
  if (!trace && !lhr) {
    throw new Error(
      "[trace-parser] parse() requires at least one of: traceJson, lhr. Both are missing or invalid."
    );
  }
  ...
  const rawEvents: RawTraceEvent[] = trace.traceEvents;
  ...
  const longTaskScripts = new Set(
    longTasks.map((t) => t.script).filter((s): s is string => s !== null)
  );
 ...
 */
function parseLHROnly(
  lhr: LighthouseLHRInput,
  vitals: ReturnType<typeof extractVitals>,
  harEntries: HAREntry[],
  url: string | null,
  parsedAt: string,
  dataQuality: ParsedTraceBottlenecks["dataQuality"]
): ParsedTraceBottlenecks {
...
function resolveTrace(input: ParseInput["traceJson"]): RawChromeTrace | null {
  if (!input) return null;
  if (typeof input === "string") {
    try {
      return JSON.parse(input) as RawChromeTrace;
    } catch {
      return null;
    }
  }
...
function resolveLHR(input: ParseInput["lhr"]): LighthouseLHRInput | null {
  if (!input) return null;
  if (typeof input === "string") {
    try {
      return JSON.parse(input) as LighthouseLHRInput;
    } catch {
      return null;
    }
  }
...
function resolveHAR(input: ParseInput["harJson"]): HAREntry[] {
  if (!input) return [];
  const har: HARLog = typeof input === "string" ? (() => {
    try { return JSON.parse(input) as HARLog; } catch { return {}; }
  })() : input;
...
function resolveDataQuality(
  ...
  hasLHR: boolean,
  hasHAR: boolean
): ParsedTraceBottlenecks["dataQuality"] {
  if (hasTrace && hasLHR && hasHAR) return "full";
  if (hasTrace && !hasLHR) return "trace";
  if (!hasTrace && hasLHR) return "lhr";
  return "partial";
}

// ─── Vitals Extraction ────────────────────────────────────────────────────────

type VitalsSnapshot = ParsedTraceBottlenecks["vitals"];
...
function extractVitals(
  lhr: LighthouseLHRInput | null,
  trace: RawChromeTrace | null
): VitalsSnapshot {
  if (lhr?.audits) {
    const a = lhr.audits;
    return {
 ...
 */
function supplementVitalsFromTrace(
...
function roundMs(val: number | null | undefined): number | null {
  return val != null ? Math.round(val) : null;
}

function roundCls(val: number | null | undefined): number | null {
