 ...
 */
import { classifyEventCategory, durToMs, extractScriptUrl, tsToMs, } from "../filters.js";
 ...
 */
export function extractLongTasks(events, renderer, lcpMs) {
    ...
    const longTasks = taskEvents.map((taskEv) => {
        ...
        for (const child of children) {
            ...
            if ((child.name === "EvaluateScript" || child.name === "FunctionCall" || child.name === "v8.execute") &&
        ...
        if (Object.keys(breakdown).length === 0) {
            ...
            if (selfUrl) {
                topScriptUrl = selfUrl;
                scriptUrlSet.add(selfUrl);
            }
        }
 ...
 * some trace formats).
 */
function extractLongTasksFromBEPairs(events, renderer, lcpMs) {
    const { navigationStart } = renderer;
 ...
 */
export function computeMainThreadSummary(events, renderer, longTasks) {
