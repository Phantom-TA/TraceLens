 ...
 */
export function extractRenderBlockersFromLHR(
  ...
  return items
    .map((item): RenderBlockingResource => {
      const url: string = item.url ?? "";
      const blockingMs: number | null =
        typeof item.wastedMs === "number" ? Math.round(item.wastedMs) : null;
      const transferSizeKB: number | null =
 ...
 */
export function extractRenderBlockersFromTrace(
  ...
  const networkRequests = events.filter(
    (ev) =>
      ev.pid === renderer.pid &&
  ...
  const blockers: RenderBlockingResource[] = [];
  ...
  for (const req of networkRequests) {
    ...
    if (resourceType !== "script" && resourceType !== "stylesheet") continue;

    // Skip preloaded resources (they don't block by definition)
    if (data["isLinkPreload"] === true) continue;
    ...
    const blockingMs =
      finishTs < fcpTs
        ? Math.round(tsToMs(finishTs, navigationStart) - tsToMs(req.ts, navigationStart))
        : null;
...
function classifyResourceType(
  url: string
): RenderBlockingResource["type"] {
  const lower = url.toLowerCase();
  if (lower.includes(".css") || lower.includes("stylesheet")) return "stylesheet";
  if (lower.includes(".js") || lower.includes("script")) return "script";
  if (lower.includes("font") || lower.includes(".woff") || lower.includes(".ttf")) return "font";
  return "unknown";
}
...
function lookupResourceSizeKB(url: string, harEntries: HAREntry[]): number | null {
