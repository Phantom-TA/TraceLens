 ...
 */
export function extractLCPFromTrace(
  ...
  if (lcpEvents.length === 0) return null;

  // The last candidate event is the authoritative LCP
  const lastCandidate = lcpEvents.reduce((prev, cur) =>
    cur.ts > prev.ts ? cur : prev
  );
  ...
  const elementType: string = data["type"] ?? "unknown";
  const resourceUrl: string | null = data["url"] ?? null;
  const nodeLabel: string | null = data["nodeLabel"] ?? data["id"] ?? null;
  const pixelSize: number = data["size"] ?? 0;
 ...
 */
export function extractLCPFromLHR(
