...
type KnownFramework = NonNullable<FrameworkDetectionResult["framework"]>;
type DetectionMethod = FrameworkDetectionResult["detectionMethods"][number];

interface FrameworkSignature {
  framework: KnownFramework;
  /** UserTiming mark name fragments to match */
  userTimingMarks: string[];
  /** Script URL substrings to match in trace events */
  scriptUrlPatterns: string[];
  /** LHR bootup-time URL substrings */
  lhrUrlPatterns: string[];
}
...
function noFramework(): FrameworkDetectionResult {
  return {
    framework: null,
    confidence: 0,
    detectionMethods: [],
...
export function detectFramework(
  ...
  const evidence = new Map<KnownFramework, {
    confidence: number;
    ...
    for (const ev of userTimingEvents) {
      ...
      if (sig.userTimingMarks.some((m) => nameLower.includes(m.toLowerCase()))) {
        addEvidence(
          ...
          0.60,
          "trace-user-timing",
          `UserTiming mark "${ev.name}" matched`
        );
  ...
  for (const sig of FRAMEWORK_SIGNATURES) {
    for (const ev of scriptEvents) {
      const url: string = ev.args?.["data"]?.url ?? ev.args?.url ?? "";
      if (!url) continue;
      if (sig.scriptUrlPatterns.some((p) => url.toLowerCase().includes(p.toLowerCase()))) {
        ...
        addEvidence(
          ...
          0.25,
          "trace-scripting",
          `Script URL "${url}" matched (${Math.round(durMs)}ms)`
        );
        break;
      }
    }
  ...
  if (lhr?.audits) {
      ...
      for (const item of bootupItems) {
        ...
        if (sig.lhrUrlPatterns.some((p) => url.includes(p.toLowerCase()))) {
          ...
          addEvidence(
            ...
            0.20,
            "lhr-bootup",
            `LHR bootup script "${item.url}" (${Math.round(scriptMs)}ms)`
          );
          break;
        }
      }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const networkItems: any[] = lhr.audits["network-requests"]?.details?.items ?? [];
    for (const sig of FRAMEWORK_SIGNATURES) {
      for (const item of networkItems) {
        ...
        if (url.endsWith(".js") && sig.lhrUrlPatterns.some((p) => url.includes(p.toLowerCase()))) {
          addEvidence(
            sig.framework,
            0.10,
            "script-url-signature",
            `Network request URL matched: "${item.url}"`
          );
          break;
        }
      }
  ...
  if (fcpMs !== null && evidence.size === 0) {
    ...
    const candidates = scriptEvents.filter(
      (ev) => ev.ts > fcpTs && ev.ts < fcpTs + 3_000_000 && (ev.dur ?? 0) > 200_000
    );
    if (candidates.length > 0) {
      const largest = candidates.reduce((a, b) => ((b.dur ?? 0) > (a.dur ?? 0) ? b : a));
      const url: string = largest.args?.["data"]?.url ?? largest.args?.url ?? "";
      ...
      for (const sig of FRAMEWORK_SIGNATURES) {
        if (url && sig.scriptUrlPatterns.some((p) => url.toLowerCase().includes(p.toLowerCase()))) {
          addEvidence(
            ...
            0.15,
            "post-fcp-heuristic",
            `Large post-FCP script (${Math.round(durToMs(largest.dur ?? 0))}ms) matched framework URL`
          );
          break;
        }
      }
      ...
      if (evidence.size === 0) {
        addEvidence(
          ...
          0.15,
          "post-fcp-heuristic",
          `Large post-FCP EvaluateScript (${Math.round(durToMs(largest.dur ?? 0))}ms) — possible framework hydration`
        );
      }
    }
  }
  ...
  if (evidence.size === 0) return noFramework();

  let bestFw: KnownFramework = "unknown";
  ...
  const methods = Array.from(winner.methods) as DetectionMethod[];
 ...
 */
export function detectFrameworkFromLHR(lhr: LighthouseLHRInput): FrameworkDetectionResult {
  if (!lhr.audits) return noFramework();

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const bootupItems: any[] = lhr.audits["bootup-time"]?.details?.items ?? [];
  const evidence = new Map<KnownFramework, { confidence: number; note: string }>();
  ...
  let bestConf = 0;
  for (const [fw, data] of evidence.entries()) {
    if (data.confidence > bestConf) { bestConf = data.confidence; bestFw = fw; }
  }
