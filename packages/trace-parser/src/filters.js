 ...
 */
export function discoverRendererThread(events) {
 ...
 */
export function filterEvents(events, renderer) {
    const filtered = [];
    for (const ev of events) {
 ...
 */
export function getEventDurationUs(ev) {
    return ev.dur ?? 0;
}
/**
 ...
 */
export function classifyEventCategory(ev) {
    const name = ev.name;
    const cat = ev.cat ?? "";
 ...
 */
export function extractScriptUrl(ev) {
