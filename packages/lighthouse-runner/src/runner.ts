 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function extractVitals(lhr: Record<string, any>): CoreWebVitals {
 ...
 */
function averageRouteRuns(runs: LighthouseRouteResult[]): AveragedMetrics {
  const successful = runs.filter((r) => r.success);
  return {
    performanceScore: mean(
      successful.map((r) => r.scores.performance.score !== null
...
async function runRoute(
  ...
  try {
    ...
    const lhConfig = buildLighthouseConfig(
      config.preset,
      config.categories,
      config.locale,
    ...
    const runnerResult = await lighthouse(
      ...
      lhConfig as Parameters<typeof lighthouse>[2]
    );
    ...
    const cats = (lhr as any).categories ?? {};
    ...
    return {
      ...
      vitals: extractVitals(lhr as any),
      artifacts,
      success: true,
    };
  } catch (err) {
    ...
    console.error(
      `[lighthouse-runner] Route failed: ${route.url} (run ${runIndex + 1})\n  ${message}`
    );
...
export async function run(
  userConfig: LighthouseRunnerConfig
  ...
  console.log(
    `[lighthouse-runner] Auditing ${config.routes.length} route(s), ` +
      `${config.runs} run(s) each, preset: ${config.preset}`
  );

  const sessionDir = initSessionDir(config.outputDir, sessionId);
  const routeSummaries: LighthouseRouteSummary[] = [];

  for (const route of config.routes) {
    const routeRuns: LighthouseRouteResult[] = [];

    for (let runIndex = 0; runIndex < config.runs; runIndex++) {
      console.log(
        `[lighthouse-runner] → ${route.url} (run ${runIndex + 1}/${config.runs})`
      );
    ...
    const summary: LighthouseRouteSummary = {
      route,
  ...
  if (!config.skipSummary) {
    ...
    const summaryPayload: LighthouseRunnerResult = {
      sessionId,
      startedAt,
      completedAt,
      durationMs,
  ...
  const runnerResult: LighthouseRunnerResult = {
    sessionId,
    startedAt,
    completedAt,
    durationMs,
  ...
  console.log(
    `[lighthouse-runner] Session complete: ${sessionId} ` +
