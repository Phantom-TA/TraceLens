...
const DEFAULT_PRESET: AuditPreset = "desktop";
const DEFAULT_FORMATS: OutputFormat[] = ["json", "html"];
const DEFAULT_RUNS = 1;
const DEFAULT_PORT = 0; // 0 = auto-assign an available port
const DEFAULT_TIMEOUT = 60_000;
const DEFAULT_LOCALE = "en";
const DEFAULT_CATEGORIES: LighthouseCategory[] = [
  "performance",
  "accessibility",
  "best-practices",
  "seo",
...
export function resolveConfig(
  config: LighthouseRunnerConfig
): ResolvedLighthouseConfig {
  if (!config.routes || config.routes.length === 0) {
    throw new Error(
      "[lighthouse-runner] config.routes must contain at least one route."
    );
  }
  ...
  if (formats.length === 0) {
    throw new Error(
      "[lighthouse-runner] config.formats must contain at least one format (\"json\" or \"html\")."
    );
  }
  ...
  const outputDir =
    ...
    path.resolve(__dirname, "../../reports/lighthouse");
