 ...
 */
export async function launchBrowser(config: ResolvedRunnerConfig): Promise<Browser> {
 ...
 */
async function applyNetworkThrottle(
  context: BrowserContext,
  ...
  try {
    ...
    console.warn(
      `[playwright-runner] Warning: failed to apply throttle profile "${profile}":`,
      err
    );
    return null;
  }
