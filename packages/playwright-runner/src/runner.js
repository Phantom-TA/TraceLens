 ...
 */
import { closeBrowser, closeContext, createContext, createPage, launchBrowser, } from "./browser.js";
import { resolveConfig } from "./config.js";
import { initRouteDir, initSessionDir, resolveArtifactPaths, slugifyRoute, writeJson, } from "./output-manager.js";
...
async function runRoute(browser, route, config, sessionDir, runIndex) {
    ...
    const routeDir = initRouteDir(sessionDir, routeLabel, config.runs > 1 ? runIndex : undefined);
    ...
    try {
        ...
        await waitForPageStability(page, route.waitForSelector, route.waitMs, config.waitTimeout);
        // ── Step 5: Extract navigation timings ────────────────────────────────────
        const timings = await extractNavigationTimings(page);
        // ── Step 6: Capture screenshot ────────────────────────────────────────────
        const screenshotResult = await captureScreenshot(page, artifactPaths.screenshot, config);
        await page.close();
        // ── Step 7: Stop and save trace ───────────────────────────────────────────
        const traceResult = await stopTrace(context.context, artifactPaths.trace, config);
