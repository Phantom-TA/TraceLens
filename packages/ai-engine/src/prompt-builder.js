 ...
 */
export function buildUserPrompt(report) {
    const { session, coreWebVitals: v, mainThread, hydration, bundle, performanceRisks, scriptingBottlenecks, renderBlockingResources, primaryBottleneck, aiSignals, dataQuality } = report;
    // Build compact vitals
    const vitals = {};
    if (v.lcp.value !== null)
        vitals.lcp = `${v.lcp.value}ms [${v.lcp.rating}]`;
    if (v.fcp.value !== null)
        vitals.fcp = `${v.fcp.value}ms [${v.fcp.rating}]`;
    if (v.tbt.value !== null)
        vitals.tbt = `${v.tbt.value}ms [${v.tbt.rating}]`;
    if (v.cls.value !== null)
        vitals.cls = `${v.cls.value} [${v.cls.rating}]`;
    if (v.tti.value !== null)
        vitals.tti = `${v.tti.value}ms [${v.tti.rating}]`;
    if (v.ttfb.value !== null)
        vitals.ttfb = `${v.ttfb.value}ms [${v.ttfb.rating}]`;
    if (v.performanceScore !== null)
        vitals.score = `${v.performanceScore}/100`;
