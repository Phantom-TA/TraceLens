...
// ─── Main Parser ───────────────────────────────────────────────────────────────
export function parseAIResponse(raw, primaryBottleneck) {
    const warnings = [];
    let parsed;
    try {
    ...
    const pbRaw = parsed["primaryBottleneck"];
    const pb = {
        ...
        evidence: ensureArray(pbRaw?.["evidence"]),
    };
    // ── Parse rootCauses ───────────────────────────────────────────────────────
    const rawCauses = ensureArray(parsed["rootCauses"]);
    const rootCauses = rawCauses.slice(0, 5).map((c, i) => ({
    ...
    const rawRecs = ensureArray(parsed["recommendations"]);
    const recommendations = rawRecs.slice(0, 7).map((r, i) => ({
    ...
    const impactRaw = parsed["estimatedImpact"];
    ...
    const confidence = {
        ...
        dataQuality: validDQ.has(dataQualityRaw)
            ? dataQualityRaw
            : "medium",
        sourcesUsed: ensureArray(confRaw?.["sourcesUsed"]),
...
// ─── Fallback Report ───────────────────────────────────────────────────────────
function buildFallbackReport(raw, primaryBottleneck) {
    return {
