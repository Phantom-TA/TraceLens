...
const VALID_PROVIDERS: AIProviderName[] = ["openai", "gemini", "anthropic", "openrouter"];
...
export function resolveAIConfig(
  ...
  const providerRaw = (
    override?.provider ??
    (process.env["TRACELENS_AI_PROVIDER"] ?? "")
  ).toLowerCase().trim() as AIProviderName;
