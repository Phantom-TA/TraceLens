 ...
 */
export async function runPipeline(config: PipelineConfig): Promise<TraceLensResult> {
  ...
  for (const route of result.routes) {
    ...
    if (route.vitals.lcp !== null) logger.metric("  LCP", route.vitals.lcp, "ms");
    if (route.vitals.fcp !== null) logger.metric("  FCP", route.vitals.fcp, "ms");
    if (route.vitals.performanceScore !== null) {
      logger.metric("  Score", Math.round((route.vitals.performanceScore ?? 0) * 100), "/100");
    }
    if (route.primaryBottleneck) logger.metric("  Bottleneck", route.primaryBottleneck);
    if (route.artifacts.bottlenecksJsonPath) logger.artifact("  Intelligence", route.artifacts.bottlenecksJsonPath);
  }
