...
import type { DuplicateInstance, DuplicatePackage } from "../types.js";
...
export function detectDuplicatePackages(
  ...
  const packageInstances = new Map<
    string,
    Map<string, { bytes: number; nestedPath: string | null }>
  >();
  ...
  for (const [packageName, locations] of packageInstances) {
    if (locations.size < 2) continue; // Only one location = no duplicate
 ...
 */
function extractLocationKey(modulePath: string, packageName: string): string | null {
