 ...
 */
export function extractRouteChunks(
  chunks: NormalizedChunk[],
  modules: NormalizedModule[]
): RouteChunk[] {
  const routeChunks: RouteChunk[] = [];

  // Build module lookup: chunkId → modules
  const chunkModuleMap = new Map<string | number, NormalizedModule[]>();
  for (const mod of modules) {
    for (const chunkId of mod.chunkIds) {
      if (!chunkModuleMap.has(chunkId)) chunkModuleMap.set(chunkId, []);
  ...
  for (const chunk of chunks) {
    ...
    if (bytesToKB(chunk.sizeBytes) < 5) continue;
 ...
 */
function inferRoutePath(names: string[]): string | null {
...
function parseChunkName(name: string): string | null {
  if (!name) return null;

  // Next.js pages directory: "pages/dashboard/index" → "/dashboard"
  if (name.startsWith("pages/")) {
    const path = name.slice("pages/".length);
    if (path === "index" || path === "_app" || path === "_document") return null;
  ...
  if (/^[A-Z][A-Za-z0-9]+Page$/.test(name)) {
    return "/" + name.replace(/Page$/, "").replace(/([A-Z])/g, (m, l, i) =>
