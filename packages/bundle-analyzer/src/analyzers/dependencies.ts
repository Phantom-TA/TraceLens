...
import type { BundleDependency, DependencyCategory } from "../types.js";
import type { NormalizedModule } from "../adapters/webpack-stats.js";

const MAX_DEPENDENCIES = 15;

// ─── Known Package Intelligence ───────────────────────────────────────────────
 ...
 */
const PACKAGE_INTEL: Record<string, PackageIntel> = {
  // Framework
 ...
 */
export function extractLargestDependencies(
  modules: NormalizedModule[],
  totalBytes: number
): BundleDependency[] {
  // Group by package name
  const packageMap = new Map<string, { bytes: number; initial: boolean }>();
  ...
  const results: BundleDependency[] = [];
 ...
 */
export function computeInitialComposition(
  modules: NormalizedModule[],
  initialSizeBytes: number,
  deps: BundleDependency[]
): import("../types.js").InitialBundleComposition {
  let frameworkBytes = 0;
  let thirdPartyBytes = 0;
  let appCodeBytes = 0;

  const notableInitialDeps: string[] = [];
  const NOTABLE_CATEGORIES: DependencyCategory[] = ["chart-library", "analytics", "ads", "ui-library", "date-library"];

  for (const mod of modules) {
    if (!mod.isInitial) continue;
...
function lookupIntel(packageName: string): PackageIntel {
  // Exact match first
  if (PACKAGE_INTEL[packageName]) return PACKAGE_INTEL[packageName]!;

  // Prefix match (handles scoped packages and sub-paths)
  for (const [key, intel] of Object.entries(PACKAGE_INTEL)) {
    if (packageName.startsWith(key)) return intel;
  }
