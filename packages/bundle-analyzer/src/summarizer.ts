...
export function buildBundleAISignals(result: Omit<BundleAnalysisResult, "aiSignals">): string[] {
  const signals: string[] = [];
  const {
    initialBundleSizeKB, asyncBundleSizeKB,
    largestDependencies, duplicatePackages, routeChunks,
    hydrationRisk, performanceSignals: ps, correlations,
    initialComposition,
  } = result;
  ...
  if (ps.thirdPartyInInitialBundle) {
    const analyticsOrAds = largestDependencies.filter(
      (d) => d.initial && (d.category === "analytics" || d.category === "ads")
    );
    if (analyticsOrAds.length > 0) {
      signals.push(
        `Analytics/ads in initial bundle: ${analyticsOrAds.map((d) => `"${d.name}"`).join(", ")} — defer after hydration.`
      );
    }
  }
  ...
  if (duplicatePackages.length > 0) {
    ...
    signals.push(
      `${duplicatePackages.length} duplicate package(s) waste ~${totalWasted}KB. Run "npm dedupe" or check yarn resolutions.`
    );
