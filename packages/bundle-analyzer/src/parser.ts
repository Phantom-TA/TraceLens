 ...
 * ```
 */
export function analyze(input: BundleAnalysisInput): BundleAnalysisResult {
  ...
  if (!stats && !sme) {
    throw new Error(
      "[bundle-analyzer] analyze() requires at least one of: webpackStats, sourceMapExplorer."
    );
  }
  ...
  if (stats) {
    ...
    const initialComposition = computeInitialComposition(
      normalized.modules,
      normalized.initialSizeBytes,
      allDeps
    );

    // ── Step 3e: Performance signals ────────────────────────────────────────
    const performanceSignals = computePerformanceSignals(
      initialSizeKB, allDeps, duplicatePackages, routeChunks
    );
    ...
    // ── Step 4: Correlate ───────────────────────────────────────────────────
    const correlations = correlateBundleSignals(
      initialSizeKB, performanceSignals, allDeps, duplicatePackages, input.traceBottlenecks
    );

    // ── Step 5: AI signals ──────────────────────────────────────────────────
    const partial: Omit<BundleAnalysisResult, "aiSignals"> = {
...
function analyzeSMEOnly(
  sme: SourceMapExplorerInput,
  input: BundleAnalysisInput,
  analyzedAt: string,
  dataQuality: BundleAnalysisResult["dataQuality"]
): BundleAnalysisResult {
  const bundles = parseSourceMapExplorer(sme);

  let initialBytes = 0;
  let asyncBytes = 0;
  const mergedPackageSizes = new Map<string, { bytes: number; initial: boolean }>();

  for (const bundle of bundles) {
    const isInitial = bundle.isInitialGuess;
    if (isInitial) initialBytes += bundle.totalBytes;
    else asyncBytes += bundle.totalBytes;
  ...
  const duplicatePackages: import("./types.js").DuplicatePackage[] = []; // Can't detect from SME alone
  const routeChunks: import("./types.js").RouteChunk[] = [];
...
function resolveWebpackStats(input: BundleAnalysisInput["webpackStats"]): WebpackStatsInput | null {
  if (!input) return null;
  if (typeof input === "string") {
    try { return JSON.parse(input) as WebpackStatsInput; } catch { return null; }
  }
  return input;
}

function resolveSourceMapExplorer(input: BundleAnalysisInput["sourceMapExplorer"]): SourceMapExplorerInput | null {
  if (!input) return null;
  if (typeof input === "string") {
    try { return JSON.parse(input) as SourceMapExplorerInput; } catch { return null; }
  }
  return input;
}
...
function resolveDataQuality(
  hasStats: boolean,
  hasSME: boolean
): BundleAnalysisResult["dataQuality"] {
  if (hasStats && hasSME) return "combined";
  if (hasStats) return "webpack-stats";
  if (hasSME) return "source-map";
  return "partial";
}
