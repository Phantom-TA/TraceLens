...
import type { SourceMapExplorerBundle, SourceMapExplorerInput } from "../types.js";
import { extractPackageInfo } from "./webpack-stats.js";

export interface SMENormalizedBundle {
  bundleName: string;
  totalBytes: number;
  isInitialGuess: boolean; // heuristic: "main", "app", "index" → initial
  packageSizes: Map<string, number>; // packageName → total bytes
  appCodeBytes: number;
 ...
 */
export function parseSourceMapExplorer(
  input: SourceMapExplorerInput
): SMENormalizedBundle[] {
  const results = input.results ?? [];
  return results.map((bundle) => normalizeSMEBundle(bundle));
}

function normalizeSMEBundle(bundle: SourceMapExplorerBundle): SMENormalizedBundle {
