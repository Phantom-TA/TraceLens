 ...
 */
export function correlateBundleSignals(
  initialSizeKB: number,
  signals: BundlePerformanceSignals,
  deps: BundleDependency[],
  duplicates: DuplicatePackage[],
  trace: BundleAnalysisInput["traceBottlenecks"]
): BundleCorrelationInsights {
...
function diagnosePrimaryIssue(
  ...
  if (initialSizeKB > 1000) return "oversized-initial-bundle";
  ...
  if (totalWasted > 100) return "duplicate-packages";
  ...
  if (signals.oversizedRouteChunks) return "oversized-route-chunks";

  // Large initial JS but no specific cause → poor code splitting
  if (initialSizeKB > 500) return "poor-code-splitting";

  // Large initial JS
  if (signals.largeInitialJS) return "oversized-initial-bundle";

  return "well-optimized";
}

function buildExplanation(
  issue: BundleIssueCategory,
  initialSizeKB: number,
  signals: BundlePerformanceSignals,
  deps: BundleDependency[],
  duplicates: DuplicatePackage[],
  ...
  switch (issue) {
    case "oversized-initial-bundle": {
      ...
      parts.push(
        `Initial JavaScript bundle is ${initialSizeKB}KB — estimated ${signals.estimatedParseMs}ms parse time on a mid-range device.`
      );
      if (topDep) parts.push(`Largest dependency: "${topDep.name}" at ${topDep.sizeKB}KB.`);
      break;
    }
    case "heavy-third-party": {
      const heavyDeps = deps.filter(
        (d) => d.initial && (d.category === "analytics" || d.category === "ads" || d.category === "chart-library")
      );
      parts.push(
        `Heavy third-party scripts in the initial bundle: ${heavyDeps.map((d) => `"${d.name}" (${d.sizeKB}KB)`).join(", ")}.`
      );
    ...
    case "duplicate-packages": {
      ...
      parts.push(
        `${duplicates.length} duplicate package(s) detected, wasting ~${totalWasted}KB. ` +
    ...
    case "oversized-route-chunks":
      parts.push(
        `One or more route chunks exceed 500KB. This means users downloading a specific page load far more JS than necessary.`
      );
      break;
    case "poor-code-splitting":
      parts.push(
        `Initial bundle is ${initialSizeKB}KB. Consider React.lazy() / dynamic imports to split code by route.`
      );
      break;
    case "well-optimized":
  ...
  if (fcpMs !== null) parts.push(`Runtime FCP: ${fcpMs}ms.`);
  if (tbtMs !== null) parts.push(`Runtime TBT: ${tbtMs}ms.`);
  if (jsBeforeFcpMs !== null) parts.push(`JS executed before FCP: ${jsBeforeFcpMs}ms.`);
