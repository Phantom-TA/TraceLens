...
export interface WebpackChunk {
  ...
  origins?: Array<{ request?: string; moduleName?: string }>;
}
...
export interface SourceMapExplorerInput {
  ...
  errors?: Array<{ bundleName: string; code: string; message: string }>;
}
...
export interface BundleAnalysisInput {
   ...
   */
  traceBottlenecks?: {
    vitals?: { fcp?: number | null; lcp?: number | null; tbt?: number | null };
    bundleSignals?: { jsBeforeFcpMs?: number; largeInitialJS?: boolean };
    scriptingBottlenecks?: Array<{ url: string; totalExecutionMs: number }>;
  };
...
export type DependencyCategory =
  ...
  | "ui-library"       // @mui, antd, chakra-ui
  | "chart-library"    // chart.js, d3, recharts, highcharts
  | "analytics"        // google-analytics, segment, mixpanel
  | "ads"              // pubads, gpt, adsense
  | "utility"          // lodash, ramda, underscore
  | "date-library"     // moment, dayjs, date-fns
  | "rich-text"        // quill, draft-js, tiptap
  | "video-player"     // video.js, plyr
  | "maps"             // google-maps, leaflet, mapbox
  | "state"            // redux, zustand, mobx, recoil
  | "icons"            // @fortawesome, react-icons, heroicons
  | "animations"       // framer-motion, gsap, anime
  | "other";
...
export interface RouteChunk {
  ...
  topDependencies: Array<{ name: string; sizeKB: number }>;
}
...
export type BundleIssueCategory =
