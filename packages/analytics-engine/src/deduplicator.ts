...
function signalPriority(signal: string): number {
  ...
  for (let i = 0; i < HIGH_PRIORITY_KEYWORDS.length; i++) {
    if (lower.includes(HIGH_PRIORITY_KEYWORDS[i].toLowerCase())) return i;
  }
  return HIGH_PRIORITY_KEYWORDS.length;
}

/** Whether a signal contains quantitative data */
function hasQuantitativeData(signal: string): boolean {
 ...
 */
function semanticKey(signal: string): string {
  return signal
    .toLowerCase()
    .replace(/[^a-z0-9 ]/g, "")
    .slice(0, 40)
...
export function deduplicateAndRankSignals(
  ...signalSets: (string[] | null | undefined)[]
): string[] {
  const allSignals: Array<{ signal: string; priority: number; hasData: boolean }> = [];
...
export function assessDataQuality(input: {
  ...
  if (input.hasPlaywright)    sources.push("playwright");
  if (input.hasLighthouse)    sources.push("lighthouse");
  if (input.hasTraceParser)   sources.push("trace-parser");
  if (input.hasBundleAnalysis) sources.push("bundle-analyzer");

  const confidence =
    sources.length >= 4 ? "high" :
