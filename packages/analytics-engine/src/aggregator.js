 ...
 */
import { normalizeCoreWebVitals, normalizeMainThread, normalizeScriptingBottlenecks, normalizeRenderBlockingResources, normalizeLCPCandidate, normalizeHydration, } from "./normalizer.js";
import { correlatePerformanceRisks, resolvePrimaryBottleneck, normalizeBundle, } from "./correlator.js";
...
export function aggregate(input) {
    ...
    const correlationSignals = performanceRisks.slice(0, 5).map((r) => `[${r.severity.toUpperCase()}] ${r.label}: ${r.impact}. Fix: ${r.recommendation}`);
    const aiSignals = deduplicateAndRankSignals(correlationSignals, bottlenecks?.aiSignals, bundle?.aiSignals);
