...
export function ratePerformanceScore(score: number | null): MetricRating {
...
function overallRating(ratings: MetricRating[]): MetricRating {
  const known = ratings.filter((r) => r !== "unknown");
  if (known.length === 0) return "unknown";
  if (known.some((r) => r === "poor")) return "poor";
  if (known.some((r) => r === "needs-improvement")) return "needs-improvement";
  return "good";
}

// ─── Core Web Vitals ───────────────────────────────────────────────────────────

export function normalizeCoreWebVitals(
  vitals: {
    fcp: number | null;
    lcp: number | null;
    tbt: number | null;
...
export function severityFromDuration(ms: number): Severity {
...
export function normalizeMainThread(
  ...
  const topLongTasks: NormalizedLongTask[] = bottlenecks.largestLongTasks
    .slice(0, 5)
    .map((t) => ({
      script: t.script,
...
function cleanScriptUrl(url: string): string {
  if (!url || url === "Unattributable") return "Unattributable";
  try {
...
export function normalizeScriptingBottlenecks(
  bottlenecks: ParsedTraceBottlenecks | null
): NormalizedScriptingBottleneck[] {
...
export function normalizeRenderBlockingResources(
  bottlenecks: ParsedTraceBottlenecks | null
): NormalizedRenderBlockingResource[] {
  if (!bottlenecks) return [];
...
export function normalizeLCPCandidate(
  bottlenecks: ParsedTraceBottlenecks | null
): NormalizedLCPCandidate | null {
  if (!bottlenecks?.lcpCandidate) return null;
...
export function normalizeHydration(
  bottlenecks: ParsedTraceBottlenecks | null
): NormalizedHydration {
  if (!bottlenecks) {
    return {
  ...
  if (h.detected && (h.durationMs ?? 0) > 1000) severity = "high";
  else if (h.detected && (h.durationMs ?? 0) > 500) severity = "medium";
  else if (bs.largeInitialJS && jsBeforeFcpMs > 3000) severity = "high";
  else if (bs.largeInitialJS && jsBeforeFcpMs > 1500) severity = "medium";
  else if (bs.largeInitialJS) severity = "low";
