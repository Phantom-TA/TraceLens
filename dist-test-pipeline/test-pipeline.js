...
const index_js_1 = require("./packages/report-engine/src/index.js");
async function main() {
    const result = await (0, index_js_1.runPipeline)({
        routes: [
    ...
    for (const route of result.routes) {
        ...
        if (a.screenshotPath)
            console.log(`      screenshot   : ${a.screenshotPath}`);
        if (a.harPath)
            console.log(`      HAR          : ${a.harPath}`);
        if (a.lighthouseJsonPath)
            console.log(`      LHR JSON     : ${a.lighthouseJsonPath}`);
        if (a.bottlenecksJsonPath)
            console.log(`      bottlenecks  : ${a.bottlenecksJsonPath}`);
    }
    console.log("\n══════════════════════════════════════════════════════════\n");
}
main().catch((err) => {
