...
exports.extractRouteChunks = extractRouteChunks;
 ...
 */
function extractRouteChunks(chunks, modules) {
    const routeChunks = [];
    ...
    for (const chunk of chunks) {
        ...
        if ((0, dependencies_js_1.bytesToKB)(chunk.sizeBytes) < 5)
            continue;
        ...
        const topDeps = [...depSizes.entries()]
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(([name, bytes]) => ({ name, sizeKB: (0, dependencies_js_1.bytesToKB)(bytes) }));
        routeChunks.push({
            route: routePath,
            chunkName: chunk.names[0] ?? String(chunk.id),
            sizeKB: (0, dependencies_js_1.bytesToKB)(chunk.sizeBytes),
