...
exports.computePerformanceSignals = computePerformanceSignals;
exports.assessHydrationRisk = assessHydrationRisk;
// ─── Thresholds ────────────────────────────────────────────────────────────────
 ...
 */
function computePerformanceSignals(initialSizeKB, deps, duplicates, routeChunks) {
    ...
    const thirdPartyInInitialBundle = initialDeps.some((d) => d.category === "analytics" || d.category === "ads");
 ...
 */
function assessHydrationRisk(initialSizeKB, deps) {
    const initialDeps = deps.filter((d) => d.initial);
    const heavyDeps = [];
    ...
    for (const dep of initialDeps) {
        ...
        if (isHeavy)
            heavyDeps.push(dep.name);
        if (shouldBeAsync && dep.initial)
            hasRenderBlockingDeps = true;
    }
