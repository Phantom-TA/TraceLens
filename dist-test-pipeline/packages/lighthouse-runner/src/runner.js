...
async function runRoute(route, config, sessionDir, runIndex) {
    ...
    const routeLabel = route.label ?? (0, output_manager_js_1.slugifyRoute)(route.url);
    const routeDir = (0, output_manager_js_1.initRouteDir)(sessionDir, routeLabel, runIndex, config.runs);
    const artifactPaths = (0, output_manager_js_1.resolveArtifactPaths)(routeDir, config.formats);
    const artifacts = {
        outputDir: routeDir,
    ...
    try {
        ...
        const lhConfig = (0, presets_js_1.buildLighthouseConfig)(config.preset, config.categories, config.locale, route.configOverrides);
        // Collect output formats for Lighthouse's own report generation
        const outputFormats = config.formats.length === 1
            ? config.formats[0]
            : config.formats;
        // ── Step 2: Launch Chrome ──────────────────────────────────────────────────
        const chromeFlags = [...presets_js_1.BASE_CHROME_FLAGS, ...config.chromeFlags];
        chrome = await (0, chrome_launcher_1.launch)({
        ...
        const runnerResult = await (0, lighthouse_1.default)(route.url, {
            port: chrome.port,
        ...
        if (Array.isArray(report)) {
            ...
            formatOrder.forEach((fmt, i) => {
                ...
                if (fmt === "json" && artifactPaths.json) {
                    (0, output_manager_js_1.writeFile)(artifactPaths.json, content);
                }
                else if (fmt === "html" && artifactPaths.html) {
                    (0, output_manager_js_1.writeFile)(artifactPaths.html, content);
                }
            });
        }
            ...
            if (fmt === "json" && artifactPaths.json) {
                (0, output_manager_js_1.writeFile)(artifactPaths.json, report);
            }
            else if (fmt === "html" && artifactPaths.html) {
                (0, output_manager_js_1.writeFile)(artifactPaths.html, report);
            }
        }
...
async function run(userConfig) {
    ...
    const sessionDir = (0, output_manager_js_1.initSessionDir)(config.outputDir, sessionId);
    ...
    if (!config.skipSummary) {
        summaryPath = path_1.default.join(sessionDir, "session-summary.json");
        ...
        (0, output_manager_js_1.writeJson)(summaryPath, summaryPayload);
    }
    const runnerResult = {
        sessionId,
        startedAt,
