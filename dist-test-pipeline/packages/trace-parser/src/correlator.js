...
exports.correlateSignals = correlateSignals;
/**
 ...
 */
function correlateSignals(lcpCandidate, longTasks, renderBlockers, hydration, bundleSignals, fcpMs, ttfbMs) {
    ...
    if (lcpMs !== null && longTasks.length > 0) {
        ...
        lcpBlockedByLongTask = longTasks.some((t) => t.startTime + t.duration >= lcpWindowStart &&
            t.startTime <= lcpMs);
    }
    // ── 2. FCP blocked by render-blocking resources ───────────────────────────
    const fcpBlockedByResources = renderBlockers.length > 0 &&
...
function diagnosePrimaryBottleneck(input) {
    const { renderBlockers, bundleSignals, hydration, longTasks, ttfbMs, lcpCandidate, fcpBlockedByResources, heavyJsBeforeFcp, hydrationDelayedLcp, } = input;
    ...
    if (heavyJsBeforeFcp && bundleSignals.jsBeforeFcpMs > 500)
        return "heavy-javascript";
    // Hydration delay causing interaction gap
    if (hydrationDelayedLcp && (hydration.durationMs ?? 0) > 300)
        return "hydration-delay";
    ...
    if (longTasks.length > 0 && longTasks[0].duration > 200)
        return "long-tasks";
    // Moderate render blocking
    if (fcpBlockedByResources)
        return "render-blocking-resources";
    // Moderate JS
    if (heavyJsBeforeFcp)
        return "heavy-javascript";
...
function buildExplanation(input) {
    ...
    switch (primaryBottleneck) {
        ...
        case "render-blocking-resources": {
            ...
            parts.push(`Render-blocking resource detected: "${top?.url}" blocks FCP by ~${top?.blockingMs ?? "?"}ms. ` +
                `The browser cannot paint until this resource is fully loaded.`);
            break;
        }
        case "heavy-javascript":
            parts.push(`Heavy JavaScript (${bundleSignals.jsBeforeFcpMs}ms) evaluated before FCP. ` +
                `The main thread is occupied with JS parsing/execution, delaying first paint.`);
            break;
        case "hydration-delay":
            parts.push(`Framework hydration delay detected (${hydration.durationMs}ms). ` +
        ...
        case "long-tasks": {
            ...
            parts.push(`Long task blocking main thread: ${top?.duration}ms ${top?.attribution} work` +
    ...
    if (lcpMs !== null)
        parts.push(`LCP: ${lcpMs}ms.`);
    if (fcpMs !== null)
        parts.push(`FCP: ${fcpMs}ms.`);
    if (hydration.detected && hydration.durationMs)
        parts.push(`Hydration: ${hydration.durationMs}ms.`);
    return parts.join(" ");
}
