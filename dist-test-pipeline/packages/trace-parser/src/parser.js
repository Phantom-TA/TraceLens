...
const long_tasks_js_1 = require("./extractors/long-tasks.js");
const lcp_js_1 = require("./extractors/lcp.js");
const render_blocking_js_1 = require("./extractors/render-blocking.js");
const hydration_js_1 = require("./extractors/hydration.js");
const scripting_js_1 = require("./extractors/scripting.js");
...
function parse(input) {
    ...
    const renderer = (0, filters_js_1.discoverRendererThread)(rawEvents);
    if (!renderer) {
    ...
    const filteredEvents = (0, filters_js_1.filterEvents)(rawEvents, renderer);
    // ── Step 5a: Long tasks + main thread ──────────────────────────────────────
    const longTasks = (0, long_tasks_js_1.extractLongTasks)(filteredEvents, renderer);
    const mainThread = (0, long_tasks_js_1.computeMainThreadSummary)(filteredEvents, renderer, longTasks);
    ...
    const renderBlockers = lhr
        ? (0, render_blocking_js_1.extractRenderBlockersFromLHR)(lhr)
        : (0, render_blocking_js_1.extractRenderBlockersFromTrace)(filteredEvents, renderer, harEntries);
    // ── Step 5c: LCP candidate ─────────────────────────────────────────────────
    const lcpFromTrace = (0, lcp_js_1.extractLCPFromTrace)(filteredEvents, renderer, harEntries, renderBlockers);
    const lcpFromLHR = lhr ? (0, lcp_js_1.extractLCPFromLHR)(lhr) : null;
    // Prefer trace LCP (more precise) but fall back to LHR
    const lcpCandidate = lcpFromTrace ?? lcpFromLHR;
    // ── Step 5d: Hydration ─────────────────────────────────────────────────────
    const fcpMs = vitals.fcp;
    const hydration = (0, hydration_js_1.detectHydration)(filteredEvents, renderer, fcpMs);
    // ── Step 5e: Scripting ─────────────────────────────────────────────────────
    const scriptingBottlenecks = (0, scripting_js_1.extractScriptingBottlenecks)(filteredEvents, renderer, longTaskScripts);
    const rendering = (0, scripting_js_1.extractRenderingTimeline)(filteredEvents, renderer);
    const bundleSignals = (0, scripting_js_1.computeBundleSignals)(filteredEvents, renderer, fcpMs, scriptingBottlenecks);
    // ── Step 6: Correlate ──────────────────────────────────────────────────────
    const correlations = (0, correlator_js_1.correlateSignals)(lcpCandidate, longTasks, renderBlockers, hydration, bundleSignals, vitals.fcp, vitals.ttfb);
    // ── Step 7: AI signals ─────────────────────────────────────────────────────
    const aiSignals = (0, summarizer_js_1.buildAISignals)({
        vitals,
        mainThread,
        longTasks,
        lcpCandidate,
...
function parseLHROnly(lhr, vitals, harEntries, url, parsedAt, dataQuality) {
    const renderBlockers = (0, render_blocking_js_1.extractRenderBlockersFromLHR)(lhr);
    const lcpCandidate = (0, lcp_js_1.extractLCPFromLHR)(lhr);
    const hydration = (0, hydration_js_1.detectHydrationFromLHR)(lhr, vitals.fcp);
    const scriptingBottlenecks = (0, scripting_js_1.extractScriptingFromLHR)(lhr);
...
function supplementVitalsFromTrace(vitals, events, renderer) {
    ...
    for (const ev of events) {
        ...
        if (vitals.fcp === null && (ev.name === "firstContentfulPaint" || ev.name === "first-contentful-paint")) {
            vitals.fcp = (0, filters_js_1.tsToMs)(ev.ts, navigationStart);
        }
        if (vitals.lcp === null && (ev.name === "LargestContentfulPaint::Candidate" || ev.name === "largestContentfulPaint::candidate")) {
            vitals.lcp = (0, filters_js_1.tsToMs)(ev.ts, navigationStart);
        }
    }
}
