...
exports.buildAISignals = buildAISignals;
const MAX_SIGNALS = 20;
/**
 * Builds the aiSignals array from all parsed bottleneck data.
 * Each signal is a concise, factual performance statement.
 */
function buildAISignals(params) {
    const signals = [];
    const { vitals, mainThread, longTasks, lcpCandidate, renderBlockers, hydration, scripting, rendering, bundle, correlations, } = params;
    ...
    if (longTasks.length > 0) {
        ...
        signals.push(`Worst long task: ${top.duration}ms of ${top.attribution}` +
    ...
    if (renderBlockers.length > 0) {
        ...
        signals.push(`${renderBlockers.length} render-blocking resource(s). Worst: "${top.url}" blocks ~${top.blockingMs ?? "?"}ms.`);
    }
    // ── Hydration ─────────────────────────────────────────────────────────────
    if (hydration.detected) {
        const fw = hydration.framework ?? "unknown framework";
        signals.push(`${fw} hydration detected: ${hydration.durationMs}ms` +
