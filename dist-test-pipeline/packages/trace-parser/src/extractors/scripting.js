...
exports.extractScriptingBottlenecks = extractScriptingBottlenecks;
exports.extractScriptingFromLHR = extractScriptingFromLHR;
exports.extractRenderingTimeline = extractRenderingTimeline;
exports.computeBundleSignals = computeBundleSignals;
 ...
 */
function extractScriptingBottlenecks(events, renderer, longTaskScripts) {
    ...
    for (const ev of mainEvents) {
        const url = (0, filters_js_1.extractScriptUrl)(ev) ?? "unknown";
        const durMs = (0, filters_js_1.durToMs)(ev.dur ?? 0);
        if (durMs < 1)
            continue; // skip sub-ms evaluations
 ...
 */
function extractScriptingFromLHR(lhr) {
 ...
 */
function extractRenderingTimeline(events, renderer) {
    ...
    for (const ev of events) {
        ...
        if (ev.name === "firstPaint" || ev.name === "first-paint") {
            firstPaintMs = (0, filters_js_1.tsToMs)(ev.ts, navigationStart);
        }
        if (ev.name === "firstContentfulPaint" || ev.name === "first-contentful-paint") {
            firstContentfulPaintMs = (0, filters_js_1.tsToMs)(ev.ts, navigationStart);
        }
        if (ev.tid !== renderer.tid || ev.ph !== "X")
            continue;
        const durMs = (0, filters_js_1.durToMs)(ev.dur ?? 0);
 ...
 */
function computeBundleSignals(events, renderer, fcpMs, scriptingBottlenecks) {
    const { navigationStart } = renderer;
    const fcpTs = fcpMs !== null ? navigationStart + fcpMs * 1000 : Infinity;
    // Sum JS evaluation time before FCP
    let jsBeforeFcpMs = 0;
    const topScriptsBeforeFcp = [];
    const preloadScripts = new Map();
    const mainEvents = events.filter((ev) => ev.pid === renderer.pid &&
        ev.tid === renderer.tid &&
        ev.ph === "X" &&
    ...
    const largeInitialJS = scriptingBottlenecks.some((s) => s.largestEvaluationMs >= LARGE_EVAL_THRESHOLD_MS) ||
        jsBeforeFcpMs > 200;
