...
exports.extractLongTasks = extractLongTasks;
exports.computeMainThreadSummary = computeMainThreadSummary;
 ...
 */
function extractLongTasks(events, renderer) {
    ...
    const taskEvents = events.filter((ev) => ev.pid === renderer.pid &&
        ev.tid === renderer.tid &&
        ev.ph === "X" &&
        (ev.name === "Task" ||
            ev.name === "RunTask" ||
    ...
    const longTasks = taskEvents.map((taskEv) => {
        const taskStart = taskEv.ts;
        const taskEnd = taskStart + (taskEv.dur ?? 0);
        const durationMs = (0, filters_js_1.durToMs)(taskEv.dur ?? 0);
        const startMs = (0, filters_js_1.tsToMs)(taskStart, navigationStart);
        // Collect all child events within this task's time window
        const children = events.filter((ev) => ev.pid === renderer.pid &&
            ev.tid === renderer.tid &&
            ev.ph === "X" &&
            ev !== taskEv &&
            ev.ts >= taskStart &&
        ...
        const breakdown = {};
        let topScriptUrl = null;
        let longestScriptDur = 0;
        for (const child of children) {
            const cat = (0, filters_js_1.classifyEventCategory)(child);
            const dur = (0, filters_js_1.durToMs)(child.dur ?? 0);
            breakdown[cat] = (breakdown[cat] ?? 0) + dur;
            // Track the longest EvaluateScript to attribute the task
            if ((child.name === "EvaluateScript" || child.name === "FunctionCall") &&
                (child.dur ?? 0) > longestScriptDur) {
                longestScriptDur = child.dur ?? 0;
                const url = (0, filters_js_1.extractScriptUrl)(child);
                if (url)
                    topScriptUrl = url;
            }
        }
        ...
        if (Object.keys(breakdown).length === 0) {
            const selfCat = (0, filters_js_1.classifyEventCategory)(taskEv);
            breakdown[selfCat] = durationMs;
            if (!topScriptUrl)
                topScriptUrl = (0, filters_js_1.extractScriptUrl)(taskEv);
        }
 ...
 * some trace formats).
 */
function extractLongTasksFromBEPairs(events, renderer) {
    const { navigationStart } = renderer;
    const mainEvents = events.filter((ev) => ev.pid === renderer.pid && ev.tid === renderer.tid);
    // Find all EvaluateScript X-events > 50ms (each is its own long task segment)
    const longScripts = mainEvents.filter((ev) => ev.ph === "X" &&
        (ev.name === "EvaluateScript" || ev.name === "v8.execute" || ev.name === "FunctionCall") &&
        (ev.dur ?? 0) >= LONG_TASK_THRESHOLD_US);
    return longScripts
        .map((ev) => ({
        script: (0, filters_js_1.extractScriptUrl)(ev),
        duration: (0, filters_js_1.durToMs)(ev.dur ?? 0),
        startTime: (0, filters_js_1.tsToMs)(ev.ts, navigationStart),
        attribution: "scripting",
        breakdown: { scripting: (0, filters_js_1.durToMs)(ev.dur ?? 0) },
    }))
        .sort((a, b) => b.duration - a.duration)
        .slice(0, MAX_LONG_TASKS);
}
 ...
 */
function computeMainThreadSummary(events, renderer, longTasks) {
    ...
    for (const task of longTasks) {
        ...
        if (task.duration > longestTaskMs)
            longestTaskMs = task.duration;
    }
    // Category breakdown from all main-thread X-events
    const categoryBreakdown = {};
    let totalMainThreadMs = 0;
    const mainEvents = events.filter((ev) => ev.pid === renderer.pid &&
        ev.tid === renderer.tid &&
        ev.ph === "X");
    for (const ev of mainEvents) {
        const durMs = (0, filters_js_1.durToMs)(ev.dur ?? 0);
        if (durMs < 0.5)
            continue; // skip sub-ms noise
        const cat = (0, filters_js_1.classifyEventCategory)(ev);
