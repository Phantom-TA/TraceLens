 ...
 */
async function createContext(browser, config, harPath) {
    const deviceProfile = (0, config_1.resolveDeviceProfile)(config.device, config.viewport);
 ...
 */
async function closeContext(context, cdpSession) {
    try {
        if (cdpSession) {
...
async function applyNetworkThrottle(context, profile) {
    const conditions = config_1.THROTTLE_CONDITIONS[profile];
    if (!conditions)
        return null;
    try {
